...
fn parse(comptime T: type, bytes: []const u8) !void {
    ...
    for (entries) |entry| {
        try handleTable(@intToPtr(SdtPtr, virt.hhdm + entry));
    }
}
...
pub fn init(rsdp_res: *limine.Rsdp.Response) !void {
    ...
    switch (rsdp.revision) {
        0 => try parse(u32, @intToPtr(SdtPtr, virt.hhdm + rsdp.rsdt_address).getData()),
        2 => try parse(u64, @intToPtr(SdtPtr, virt.hhdm + rsdp.xsdt_address).getData()),
