...
fn signatureValue(sdt: anytype) u32 {
    return std.mem.readInt(u32, sdt[0..4], .little);
}
        ...
        signatureValue("APIC") => {
            ...
            while (data.len >= 2) {
                ...
                switch (kind) {
                    ...
                    1 => apic.handleIoApic(
                        record_data[0],
                        std.mem.readInt(u32, record_data[2..6], .little),
                        std.mem.readInt(u32, record_data[6..10], .little),
                    ),
                    2 => apic.handleIoApicIso(
                        record_data[0],
                        record_data[1],
                        std.mem.readInt(u32, record_data[2..6], .little),
                        std.mem.readInt(u16, record_data[6..8], .little),
                    ),
...
pub fn init(rsdp_res: *limine.RsdpResponse) !void {
    const rsdp = @as(RsdpPtr, @ptrCast(rsdp_res.address));
