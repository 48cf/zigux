...
const Slot = struct {
    ...
    state: SlotState,
    devices: std.BoundedArray(usb.Device, 8),
            ...
            32 => {
                ...
                switch (slot.state) {
                    ...
                    .identify => {
                        ...
                        var best_devices: ?std.BoundedArray(usb.Device, 8) = null;
                        for (0..num_configurations) |_| {
                            const config_descriptor = virt.asHigherHalf([*]u8, slot.descriptor_buffer + offset);
                            const total_length = std.mem.readInt(u16, config_descriptor[2..4], .little);
                            offset += total_length;

                            const devices = usb.identifyDevice(config_descriptor[0..total_length]) catch continue;
