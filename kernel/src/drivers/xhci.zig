...
const Device = struct {
    const HidDevice = struct {
        ...
        fn scancodeToKeyLocation(scancode: usize) ?input.KeyLocation {
            ...
            if (endpoint_addr == self.poll_ep.address) {
                ...
                for (0..8, 0xE0..) |i, scancode| {
                    ...
                    if (state != previous_state) {
                        ...
                        _ = input.enqueueKeyboardEvent(.{
                            .location = scancodeToKeyLocation(scancode).?,
                            .pressed = state,
                        });
                    }
                }
                ...
                for (0x0..0xE0) |scancode| {
                    ...
                    if (std.mem.indexOfScalar(u8, scancodes, @intCast(scancode)) == null) {
                        ...
                        _ = input.enqueueKeyboardEvent(.{
                            .location = scancodeToKeyLocation(scancode) orelse continue,
                            .pressed = false,
                        });
                    }
                }
                ...
                for (scancodes) |scancode| {
                    if (scancode != 0 and !self.key_state.get(scancode)) {
                        ...
                        _ = input.enqueueKeyboardEvent(.{
                            .location = scancodeToKeyLocation(scancode) orelse {
