...
const RamFSFile = struct {
    ...
    fn read(vnode: *vfs.VNode, buffer: []u8, offset: usize, _: usize) vfs.ReadError!usize {
        ...
        @memcpy(buffer[0..bytes_read], self.data.items[offset .. offset + bytes_read]);
        return bytes_read;
    }
    ...
    fn stat(vnode: *vfs.VNode, buffer: *abi.stat) vfs.StatError!void {
        ...
        buffer.st_ino = @intCast(vnode.inode);
        buffer.st_mode = 0o777 | abi.S_IFREG;
        buffer.st_size = @intCast(self.data.items.len);
        buffer.st_blksize = std.mem.page_size;
        buffer.st_blocks = @intCast(std.mem.alignForward(usize, self.data.items.len, std.mem.page_size) / std.mem.page_size);
    }
};
...
const RamFSDirectory = struct {
    ...
    fn open(vnode: *vfs.VNode, name: []const u8, _: usize) vfs.OpenError!*vfs.VNode {
        const self: *@This() = @fieldParentPtr("vnode", vnode);
    ...
    fn readDir(vnode: *vfs.VNode, buffer: []u8, offset: *usize) vfs.ReadDirError!usize {
        ...
        while (offset.* < self.children.items.len) : (offset.* += 1) {
            ...
            dir_ent.d_off = 0;
            dir_ent.d_ino = @intCast(vnode.inode);
            dir_ent.d_reclen = @intCast(real_size);
            dir_ent.d_type = switch (child.kind) {
                .file => abi.DT_REG,
                .directory => abi.DT_DIR,
                .symlink => abi.DT_LNK,
                .character_device => abi.DT_CHR,
                .block_device => abi.DT_BLK,
                .fifo => abi.DT_FIFO,
                .socket => abi.DT_SOCK,
            };
...
const RamFS = struct {
    ...
    fn allocateInode(self: *vfs.FileSystem) vfs.OomError!u64 {
        const fs: *@This() = @fieldParentPtr("filesystem", self);
